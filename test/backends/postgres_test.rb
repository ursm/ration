...
class PostgresBackendTest < Ration::Test
  POLL_INTERVAL = 0.05
  POP_TIMEOUT   = 5

  def database_url
    ENV['RATION_TEST_DATABASE_URL']
  end

  def setup
    super
    skip 'set RATION_TEST_DATABASE_URL to run Postgres tests' unless database_url
  ...
  def test_payload_too_large_raises_before_notify
  ...
  def test_through_hub_fans_out_to_subscribers
    ...
    # ensure both subscriptions are registered before publishing
  ...
  def test_invalid_json_is_logged_and_skipped
    ...
    begin
      ...
      begin
        conn.exec_params('SELECT pg_notify($1, $2)', [@channel, 'not json'])
        conn.exec_params('SELECT pg_notify($1, $2)', [@channel, '{"ok":true}'])
      ensure
        conn.close
      end
